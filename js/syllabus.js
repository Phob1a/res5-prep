// Canonical RES5 syllabus — SCI official Contents (27 items). 编号 s01..s27 = 官方 [1]..[27]。
...
const BY_ID = new Map(SYLLABUS.map(s => [s.id, s]));
export function isValidSyllabusItemId(id) { return BY_ID.has(id); }
export function getItem(id) { return BY_ID.get(id) || null; }
export function partItems(part) { return SYLLABUS.filter(s => s.part === part); }
